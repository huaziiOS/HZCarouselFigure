...
/**
 ...
 UIScrollView + UIImageView 实现无限轮播
 
 实现思路: 在原图片数组的基础上, 分别在数组开始处插入一张原图片数组的最后一张图片, 末尾处插入原图片数组的第一张图片, 组成一个新的图片数组, 这个新图片数组.count = 原数组.count + 2
 
 使用方法: 直接将此文件导入到你的工程中, 然后创建使用就可以了
